    ...
    for (let i = 0; i < totalPlaylists; i++) {
        document.querySelectorAll(".playlist")[i].addEventListener("click", function () { //sempre que clicar numa música
            ...
            record[i].set({ //define o novo record
                ...
                resolution: map(userPlaylists[i].tracks.total, 0.0, 500, 30, 100), // número de "vértices"
                rad: 70,  //tamanho
                round: map(userPlaylists[i].average_features.energy, 0.0, 1.0, 90, 0), //quanto maior o valor, mais espalmada
                nAmp: map(userPlaylists[i].average_features.loudness, -60, 0, 0.1, 1), // valor=1 -> redonda
                t: 0,
                tChange: map(userPlaylists[i].average_features.danceability, 0.0, 1.0, 0.01, 0.2) // dança do objeto
            });
        ...
        if(recordList.isEmpty() === false) {
            ...
            currentRecord.whenReady(function () {
                ...
                addNewFlower(currentRecord.get('playlist'), currentRecord.get('px'), currentRecord.get('py'), currentRecord.get('largura'), currentRecord.get('color'),
                    currentRecord.get('resolution'), currentRecord.get('rad'), currentRecord.get('round'), currentRecord.get('nAmp'),
                    currentRecord.get('t'), currentRecord.get('tChange'));
            });
        }
    }, true);
}

function addNewFlower(name, px, py, largura, color, resolution, rad, round, nAmp, t, tChange) {
    newMountain = new classMountain(name, px, py, largura, color, resolution, rad, round, nAmp, t, tChange);
...
class classMountain {
    ...
    constructor(name, px, py, largura, color, resolution, rad, round, nAmp, t, tChange) {
        this.name = name;
        this.px = px;
        this.py = py;
    ...
    display() {
        ...
        for (let b=1; b<=2; b++) {
            for (let a = -1; a <= TWO_PI; a += TWO_PI / this.resolution) {
                this.nVal = map(noise(cos(a), sin(a), this.t), 0.0, 1.0, this.nAmp, 1.0);
